...
2. **탐욕적 선택 속성(greedy choice property)**
   ...
   $A$ 중에서 최적해가 있음을 보이기 위해서는 $A^c \rightarrow A$가 손해가 아님을 보이면 된다.
   ...
   2. $A^c$에 최적해가 존재한다고 가정
      ...
      $A^c \rightarrow A$ 는 손해가 아니므로, $A$인 최적해도 존재.

   위의 증명을 통해 $A$에는 최적해가 존재함을 알 수 있다.

   $A$의 모든 해는 $G$를 포함하므로, $G$를 포함하는 최적해가 있음을 알 수 있다.

   > 즉 탐욕적 선택($G$)을 포함하지 않는 해를 $A^c$ 에서 포함하는 해 $A$로 바꾸었을 때 손해가 아니라면 
